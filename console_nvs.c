...
static int nvs_print_command(int argc, char **argv)
{
    ...
    if(argc == 1) {
        ...
        if (esp_partition_table_verify(partitions, true, &num_partitions) != ESP_OK) {
		...
		for (int i = 0; i < num_partitions; i++) {
			...
			if(partition->subtype == 0x02) {
				...
				res = nvs_entry_find((const char*)partition->label, NULL, NVS_TYPE_ANY,&it);
				while(res == ESP_OK) {
        ...
        res = nvs_entry_find(argv[1], NULL, NVS_TYPE_ANY,&it);
		while(res == ESP_OK) {
            nvs_entry_info_t info;
            nvs_entry_info(it, &info);
            if(strcmp(namespace_name,info.namespace_name)!=0) {
        ...
        res = nvs_entry_find(argv[1], argv[2], NVS_TYPE_ANY,&it);
		while(res == ESP_OK) {
            nvs_entry_info_t info;
            nvs_entry_info(it, &info);
            if(strcmp(namespace_name,info.namespace_name)!=0) {
...
/**
 * @brief Registers the nvs command.
 *
 * @return
 *          - esp_err_t
 */
